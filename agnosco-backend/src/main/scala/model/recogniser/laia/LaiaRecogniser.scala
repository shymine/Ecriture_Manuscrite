...
//		})
	...
	/**
	  ...
	  */
	override def recognize(samples: Iterable[Example]): Iterable[Example] = { List.empty }
	...
	laia-create-model \
	  ...
	  -- 1 64 20 model.t7;
	 */
	def init(symbol: Int): Unit = {
		val ret: Int = s"laia-docker create-model 1 $height $symbol model/model.t1".!
		if(ret != 0) {
