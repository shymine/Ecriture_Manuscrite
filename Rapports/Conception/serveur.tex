...
\begin{center}
\includegraphics[scale=0.3]{assets/UML_data.pdf}
\end{center}
\end{mdframed}

L'attribut \texttt{Recognizer} est converti en \texttt{String} dans la base de données en passant par un \textit{enum} recensant les différents reconnaisseurs existant. Ces classes de données sont liées à la structure de la base de données et seront donc expliquées plus profondément dans cette partie.
